...
class WP_Security_AI {
    ...
    private function analyze_file_changes() {
        ...
        $analysis = $this->openai_analyze(array(
    ...
    private function analyze_user_behavior() {
        ...
        return $this->openai_analyze(array(
    ...
    private function enrich_ip_data($attack_data) {
        ...
        foreach ($attack_data['ips'] as &$ip) {
            $response = wp_remote_get('https://api.abuseipdb.com/api/v2/check?ipAddress=' . $ip, array(
                'headers' => array(
    ...
    private function update_cloudflare_rules($ips) {
        ...
        foreach ($ips as $ip => $risk_score) {
            wp_remote_post("https://api.cloudflare.com/client/v4/zones/{$zone_id}/firewall/rules", array(
                'headers' => array(
